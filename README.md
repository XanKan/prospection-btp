...
- **Pipeline de suivi** (bouton « Pipeline » en haut à droite) : chaque entreprise contactée — manuellement depuis un résultat, ou automatiquement via l'export Brevo — passe par un statut (Contacté, Relancé, RDV programmé, Client, Perdu), avec compteurs par statut, filtre et export CSV dédié. Une entreprise déjà dans le pipeline ne réapparaît jamais dans une recherche suivante.
